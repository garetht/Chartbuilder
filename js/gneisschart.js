...
Gneiss.defaultGneissChartConfig = {
  ...
  columnStructure: function(group) {
    var that = this;
    var columnGroup = group.append("g")
                           .attr("class", "column-group");

    columnGroup.append("rect")
         .attr("width", that.columnWidth())
         .attr("height", function(d, i) {
           yAxisIndex = d3.select(this.parentNode.parentNode).data()[0].axis;
           return Math.abs(that.yAxis()[yAxisIndex].scale(d)-that.yAxis()[yAxisIndex].scale(Gneiss.helper.columnXandHeight(d,that.yAxis()[yAxisIndex].scale.domain())))
         })
         .attr("x", function(d,i) {
           return that.xAxis().scale(that.xAxisRef()[0].data[i])  - that.columnWidth()/2
         })
         .attr("y",function(d,i) {
            yAxisIndex = d3.select(this.parentNode.parentNode).data()[0].axis;
            return (that.yAxis()[yAxisIndex].scale(d)-that.yAxis()[yAxisIndex].scale(Gneiss.helper.columnXandHeight(d,that.yAxis()[yAxisIndex].scale.domain()))) >= 0 ? that.yAxis()[yAxisIndex].scale(Gneiss.helper.columnXandHeight(d,that.yAxis()[yAxisIndex].scale.domain())) : that.yAxis()[yAxisIndex].scale(d)
        })
  },
...
function Gneiss(config)
{
  ...
  this.columnStructure = function Gneiss$columnStructure(w) {
    if (!arguments.length) {
  ...
  this.setXScales = function Gneiss$setXScales() {
    ...
    */
    ...
    if(g.isBargrid()) {
      ...
      left = p.left + halfColumnWidth + ((g.yAxis().length == 1) ? 0 : d3.selectAll("#rightAxis.yAxis g:not(.topAxisItem) text")[0].pop().getBoundingClientRect().width + g.axisBarGap());
      right = g.width() - p.right - d3.selectAll("#leftAxis.yAxis g:not(.topAxisItem) text")[0].pop().getBoundingClientRect().width - halfColumnWidth - g.axisBarGap();
      rangeArray = [left,right];
    }
    else if(!g.allowAxisOverlap()) {
      try {
        left = p.left + ((g.yAxis().length == 1) ? 0 : d3.selectAll("#rightAxis.yAxis g:not(.topAxisItem) text")[0].pop().getBoundingClientRect().width);
        right = g.width() - p.right - d3.selectAll("#leftAxis.yAxis g:not(.topAxisItem) text")[0].pop().getBoundingClientRect().width - g.dotRadius();
        rangeArray = [left,right];
      }
      catch(e){
    ...
    */
    ...
    if(first) {
      ...
      g.legendItemContainer = g.chartElement().append("g")
        ...
        var barIterator = columnGroups.selectAll("g.bar-iterator")
                                      .data(function(d, i) {
                                        return d.data;
                                      })
                                      .enter();
      ...
      if(g.isBargrid()) {
        ...
        d3.selectAll(".column-group").remove()

        var rectIterator = columnGroups.selectAll("g.bar-iterator")
                                       .data(function(d, i) {
                                        return d.data;
                                       });
        var enteredRectIterator = rectIterator.enter();

        g.columnStructure().bind(g)(enteredRectIterator);
        ...
        lineSeriesDotGroups
          ...
          .attr("fill", function(d,i){return d.color? d.color : colors[i]})
          .attr("stroke", function(d,i){
            // old light/dark theme difference
            return d.color? d.color : colors[i]
            // else {
            //   var colorIndex = d.color ? colors.indexOf(d.color.toUpperCase()) : i;
            //   var color = colorIndex < 4 ? colors[colorIndex + 4] : d.color;
